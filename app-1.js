...
let ctx=null, recording=null, currentChart=null, practice=null, scheduler=null, chosenAudio=null, audioEl=null; let editMode=false, selectedEditNote="";
...
function tone(freq,dur,gain,when,type="sine",pan=0){
  const a=audio(),o=a.createOscillator(),g=a.createGain(),p=a.createStereoPanner?a.createStereoPanner():null;
  ...
  o.connect(g);if(p){p.pan.value=pan;g.connect(p);p.connect(a.destination)}else g.connect(a.destination);o.start(when);o.stop(when+dur)
}
function noise(dur,gain,when,pan=0,hp=300){
  const a=audio(),b=a.createBuffer(1,Math.max(1,Math.floor(a.sampleRate*dur)),a.sampleRate),d=b.getChannelData(0);
  ...
  s.connect(f);f.connect(g);if(p){p.pan.value=pan;g.connect(p);p.connect(a.destination)}else g.connect(a.destination);s.start(when);s.stop(when+dur)
}
function playHit(type,when=null){
  const a=audio(),t=when??a.currentTime,pan=type.endsWith("L")?-.35:type.endsWith("R")?.35:0;
  if(type.startsWith("don")||type==="bigDon"){tone(type==="bigDon"?105:126,.16,type==="bigDon"?.42:.3,t,"sine",pan);tone(80,.21,.18,t,"triangle",pan);noise(.06,.07,t,pan,180)}
  else{tone(type==="bigKa"?820:980,.055,type==="bigKa"?.2:.14,t,"square",pan);noise(.045,type==="bigKa"?.2:.13,t,pan,1500)}
}
